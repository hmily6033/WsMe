...
import com.ws.wsme.WsBaseActivity;
import com.ws.wsme.R;
import com.ws.wsme.http.common.C;
import com.ws.wsme.http.db.DBHelper;
import com.ws.wsme.http.utils.Encoder;
import com.ws.wsme.http.utils.Log;
import com.ws.wsme.http.utils.Network;
import com.ws.wsme.http.utils.ThreadPoolUtils;
import com.ws.wsme.http.utils.TimeCompare;
...
public class AnsynHttpRequest {
	 ...
	 */
	public static void requestByGet(Context context,String geturl,
...
//		requestUrl = requestUrl.substring(0,requestUrl.length()-1);
		...
		if (!Network.checkNetWork(context)) { // 如果不可用 看本地否有数据
			sendBroadcastReceiverMessage(context, R.string.network_show_connectionless);
			callBack.back(null, url);
			((WsBaseActivity) context).checkNetWorkShowLog(1);
			return;
		}
		// 异步请求数据
		doAsynRequest(POST, map ,context, callBack, requestUrl, isCache, isShowDialog, url);
